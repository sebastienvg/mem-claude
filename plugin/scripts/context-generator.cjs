...
`+JSON.stringify(o,null,2):u=" "+this.formatData(o));let m="";if(n){let{sessionId:E,memorySessionId:T,correlationId:b,..._}=n;Object.keys(_).length>0&&(m=` {${Object.entries(_).map(([f,C])=>`${f}=${C}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${c}${s}${m}${u}`;if(this.logFilePath)try{(0,y.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",m={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,m,n),o}},l=new G;var Ot={};function ht(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,ce.fileURLToPath)(Ot.url))}var bt=ht(),R=I.get("CLAUDE_MEM_DATA_DIR"),W=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,de.homedir)(),".claude"),Bt=(0,h.join)(R,"archives"),Gt=(0,h.join)(R,"logs"),Wt=(0,h.join)(R,"trash"),Yt=(0,h.join)(R,"backups"),Vt=(0,h.join)(R,"modes"),le=(0,h.join)(R,"settings.json"),ue=(0,h.join)(R,"claude-mem.db"),Kt=(0,h.join)(R,"vector-db"),qt=(0,h.join)(R,"observer-sessions"),Jt=(0,h.join)(W,"settings.json"),Qt=(0,h.join)(W,"commands"),zt=(0,h.join)(W,"CLAUDE.md");function me(r){(0,pe.mkdirSync)(r,{recursive:!0})}function _e(){return(0,h.join)(bt,"..")}var $=class{db;constructor(e=ue){e!==":memory:"&&me(R),this.db=new Ee.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.createConversationHistoryTable()}initializeSchema(){this.db.run(`
    ...
    `).run(e.content_session_id,e.prompt_number,e.prompt_text,e.created_at,e.created_at_epoch).lastInsertRowid}}createConversationHistoryTable(){this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(21)||(l.debug("DB","Creating conversation_history table"),this.db.run(`

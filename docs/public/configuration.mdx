...
Claude-mem uses git remotes to create portable project identifiers that work across machines and worktrees.

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `CLAUDE_MEM_GIT_REMOTE_PREFERENCE` | `origin,upstream`          | Git remote priority order (comma-separated) |

When a repository has multiple remotes, the first matching remote in the preference list is used to generate the project identifier.
...
This prioritizes `upstream` over `origin`, useful for fork workflows where `upstream` points to the canonical repository.

**How it works:**
- Project identifier format: `github.com/user/repo` (normalized from SSH or HTTPS URLs)
- Falls back to folder basename if no git remote is available
- Automatic alias registration links old folder-based names to new git-based identifiers

### Agent Settings

For multi-agent deployments where multiple Claude instances share memory:

| Setting                              | Default   | Description                           |
|--------------------------------------|-----------|---------------------------------------|
| `CLAUDE_MEM_AGENT_DEFAULT_VISIBILITY` | `project` | Default observation visibility level  |
...
See [Multi-Agent Architecture](/multi-agent) for full documentation on visibility levels and security.
...
- [Multi-Agent Architecture](/multi-agent) - Share memory across agents
